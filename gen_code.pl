...
my %files = (
	tokens      => [ "tokens.list"      => "auto_tokens.h"      ],
	clierrs     => [ "clierrs.list"     => "auto_clierrs.h"     ],
	banner      => [ "banner.txt"       => "auto_banner.h"      ],
	banner_wide => [ "banner_wide.txt"  => "auto_banner_wide.h" ],
);
...
sub build_tokens_h__print
{
	...
	print $fo "#define CLI_TOKEN_INTEGER 100\n";
	print $fo "#define CLI_TOKEN_STRING  101\n";
	print $fo "\n";
...
sub build_tokens_h
{
	...
	# load tokens
	open($fi, "<", $files{tokens}[0]) or die "Cannot open '$files{tokens}[0]': $!";
	$tokens = { };
	while(my $l = <$fi>) {
		chomp $l;
	...
	# dump tokens
	open($fo, ">", $files{tokens}[1]) or die "Cannot open '$files{tokens}[1]': $!";
	auto_header $fo;
...
sub build_clierrs_h
{
	...
	# read errors
	open($fi, "<", $files{clierrs}[0]) or die "Cannot open '$files{clierrs}[0]': $!";
	$max_len=0;
	while(my $l = <$fi>) {
		chomp $l;
		next if($l =~ /^\s*(#.*)?$/); # ignore comments
	...
	# print strings
	open($fo, ">", $files{clierrs}[1]) or die "Cannot open '$files{clierrs}[1]': $!";
	auto_header $fo;
...
sub build_text_h
{
	...
	# read text
	open($fi, "<", $files{$file}[0]) or die "Cannot open '$files{$file}[0]': $!";
	$max_len=0;
	while(my $l = <$fi>) {
		chomp $l;
	...
	# print
	open($fo, ">", $files{$file}[1]) or die "Cannot open '$files{$file}[1]': $!";
	auto_header $fo;
...
build_text_h "banner";
build_text_h "banner_wide";
build_text_h "clihelp";
